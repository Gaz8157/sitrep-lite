...
def _connect(path: str | None = None) -> sqlite3.Connection:
    conn = sqlite3.connect(path or str(paths.PANEL_DB), isolation_level=None, check_same_thread=False)
...
def migrate(path: str | None = None) -> int:
    target = path or str(paths.PANEL_DB)
...
def get_conn(path: str | None = None) -> Iterator[sqlite3.Connection]:
    conn = _connect(path or str(paths.PANEL_DB))
    try:
        yield conn
    finally:
        conn.close()
